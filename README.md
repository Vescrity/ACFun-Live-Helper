...
| 模式 | 命令 | 构建依赖 | 运行时依赖 | 体积 |
|------|------|---------|-----------|------|
| **WebUI**（默认） | ① `npm run build` → ② `go build` | Node.js 18+（构建前端） | 无（纯 Go 静态链接，前端已嵌入） | ~20MB |
| **Wails 桌面版** | `go build -tags 'wails webkit2_41'` | Node.js 18+ + Wails CLI | 需 webkit2gtk-4.1 + GTK3 | ~20MB |
...
|------|------|
| Go 1.21+ | 编译用 |
| Node.js 18+ | 构建前端用（仅构建时需要，运行二进制不需要） |
| 浏览器 | Chrome / Firefox / Edge 等现代浏览器 |

> 构建完成后得到的二进制是自包含的，不依赖 Node.js、`dist/` 或 `public/` 目录。
...
**产物**：`build/bin/aclivehelper-webui`（约 20MB，纯静态链接）

该二进制已完整内嵌前端页面、弹幕覆盖层 HTML 和静态资源，**无需 `dist/`、`public/` 或任何文件放在旁边**，单文件复制到任意目录即可运行：

```bash
# 复制到任意位置运行
cp build/bin/aclivehelper-webui /home/user/my-tools/
cd /home/user/my-tools/
./aclivehelper-webui

# 或自定义端口
./aclivehelper-webui --port 8080
```

自动打开浏览器访问 `http://127.0.0.1:15369`。

> **提示**：若需分发给其他用户，只需分发该二进制文件即可。
>
> 也可使用打包脚本一键完成（构建 + 编译 + 压缩打包）：
> ```bash
> ./scripts/build-webui.sh
> # 输出: build/bin/aclivehelper-webui + aclivehelper-*-webui-*.tar.gz
> ```
...
|------|-----------|-------------|
| CGO | ❌ 无 | ✅ 有 |
| 构建依赖 | Node.js 18+ | Node.js 18+ + Wails CLI |
| 运行时依赖 | 仅浏览器 | webkit2gtk / WebView2 |
| 二进制体积 | ~20MB | ~20MB |
| 分发方式 | **单文件**，复制即用 | 需动态库支持 |
| 链接方式 | 静态链接 | 动态链接 |
...
├── embed_webui.go        # WebUI 嵌入前端静态资源             [build tag: !wails]
├── embed_wails.go        # Wails 模式空桩                     [build tag: wails]
...
Windows 用户下载 `ACFun Live Helper.exe` 双击即可运行。Linux 用户可下载 WebUI 版二进制（单文件，无需任何系统库）。

首次启动会在 `%AppData%\aclivehelper` 创建账号配置目录（Windows）或 `~/.config/ACFun Live Helper`（Linux）。
...
- **Linux 上无法启动 Wails 版**：确认已安装 webkit2gtk-4.1（系统没有 4.0 版本），使用 `npm run wails:build` 自动携带 `webkit2_41` tag。推荐直接使用 **WebUI 模式**（单二进制分发，无需任何系统库）：按照[构建说明](#webui-版无-cgo-依赖单文件分发)编译即可。
